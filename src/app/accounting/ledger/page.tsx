...
function LedgerPage() {
  ...
  const filteredParties = parties.filter(party => {
    const matchesSearch = 
      ...
      (party.panNumber && party.panNumber.toLowerCase().includes(searchQuery.toLowerCase())) ||
      (party.phone && party.phone.includes(searchQuery));
    
    const matchesType = partyType === null || (party.businessType && party.businessType.toLowerCase() === partyType);
  ...
  const sortedParties = [...filteredParties].sort((a, b) => {
    if (sortField === 'name') {
      ...
      // Sort by balance
      const balanceA = a.outstandingBalance || 0;
      const balanceB = b.outstandingBalance || 0;
      return sortOrder === 'asc' 
        ? balanceA - balanceB
        : balanceB - balanceA;
    }
  ...
  const customerCount = parties.filter(p => p.businessType === 'Customer').length;
  const supplierCount = parties.filter(p => p.businessType === 'Supplier').length;
    ...
    <Container maxWidth="xl" sx={{ mt: 4, mb: 4 }}>
      <Container maxWidth="xl" sx={{ mt: { xs: 2, md: 4 }, mb: { xs: 2, md: 4 } }}>
        ...
        <Grid container spacing={3}>
                      ...
                      >
                        <CardContent sx={{ p: 2, '&:last-child': { pb: 2 } }}>
                          ...
                          <Typography variant="body2" color="text.secondary" noWrap>
                            {party.panNumber || party.phone || 'No additional details'}
                          </Typography>
                          ...
                          <Box sx={{ 
                            ...
                            <Chip 
                              label={party.businessType || 'unknown'} 
                              size="small" 
                              color={party.businessType === 'Customer' ? 'primary' : 'secondary'} 
                              variant="outlined"
                              sx={{ flexShrink: 0 }}
                            />
                            {party.outstandingBalance !== undefined && (
                              <Typography 
                                variant="body2" 
                                fontWeight="medium"
                                sx={{ 
                                  color: (party.outstandingBalance || 0) > 0 ? 'success.main' : (party.outstandingBalance || 0) < 0 ? 'error.main' : 'text.secondary',
                                  ml: 'auto'
                                }}
                              >
                                {(party.outstandingBalance || 0) > 0 ? `₹${party.outstandingBalance} CR` : (party.outstandingBalance || 0) < 0 ? `₹${Math.abs(party.outstandingBalance || 0)} DR` : '₹0'}
                              </Typography>
                            )}
                          </Box>
                        </CardContent>
          ...
          {/* Ledger Display */}
          <Grid size={{ xs: 12, md: 8, lg: 9 }} id="ledger-section">
            <Paper 
              elevation={2} 
              sx={{ 
                height: '100%',
                  ...
                  <Box sx={{ 
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center' }}>
                      ...
                      <Box>
                        ...
                        <Typography variant="body2" color="text.secondary">
                          {selectedParty.businessType ? `${selectedParty.businessType.charAt(0).toUpperCase()}${selectedParty.businessType.slice(1)}` : 'Party'} • {selectedParty.panNumber || 'No PAN'}
                        </Typography>
                      </Box>
                    </Box>
                    <Box sx={{ display: 'flex', gap: 1 }}>
