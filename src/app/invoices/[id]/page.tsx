...
import {
  ...
  Print as PrintIcon,
  Visibility as VisibilityIcon, // Added VisibilityIcon
  VisibilityOff as VisibilityOffIcon // Added VisibilityOffIcon
...
import SimpleInvoiceView from '../components/SimpleInvoiceView'; // Import SimpleInvoiceView
...
export default function InvoiceDetailPage() {
  ...
  const [showSimpleView, setShowSimpleView] = useState(true); // State for toggling view
    ...
    const fetchInvoice = async () => {
      try {
        ...
        if (!id) {
          ...
          setLoading(false); // Ensure loading stops if ID is missing
          return;
        }
    ...
    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
      ...
      {!loading && invoice && (
        <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}> {/* Added alignItems */}
          {/* Toggle Button for Simple View */}
          <Tooltip title={showSimpleView ? "Show Full View" : "Show Simple View"}>
            <IconButton onClick={() => setShowSimpleView(!showSimpleView)} size="small">
              {showSimpleView ? <VisibilityOffIcon /> : <VisibilityIcon />}
            </IconButton>
          </Tooltip>
  ...
  return (
   <DashboardLayout>
      <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
        ...
        {/* Conditionally render based on showSimpleView state */}
        {showSimpleView ? (
