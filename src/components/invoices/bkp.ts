...
import QuickProductSearch from '@/components/invoices/QuickProductSearch';
        ...
        <TabPanel value={activeTab} index={0}>
          ...
          onChange={(_, newValue) => {
            ...
            if (newValue?.id) {
              setTimeout(() => {
                setActiveTab(1);
                // Focus the QuickProductSearch input after tab switch
                setTimeout(() => {
          ...
          <Box sx={{ 
            ...
            {productsError ? (
              ...
              <Box sx={{ width: '100%' }}>
                ...
                maxItems={25}
                partyDiscounts={selectedParty?.categoryDiscounts || {}}
                onAddToCart={(product, quantity) => {
                  let discount = 0;
                  let discountType: 'none' | 'category' | 'product' | 'custom' = 'none';
                  const margin = selectedParty ? getCategoryDiscountDetails(selectedParty, product.category || '').dp : 0;
                  
                  if (selectedParty) {
                    const { discount: categoryDiscount } = getCategoryDiscountDetails(selectedParty, product.category || '');
                    const productDiscount = selectedParty.productDiscounts?.[product.id] || 0;
                  ...
                  const gstRate = selectedParty?.gstRate || 0;
                  // Apply category discount + category DP to the final price immediately
                  const finalPrice = parseFloat((product.price * (1 - discount/100) * quantity * (1 + gstRate / 100) + (product.price * quantity * margin / 100)).toFixed(2));
                ...
                onCreateNew={(searchText) => {
                  handleOpenProductDialog(searchText, true);
                }}
                onRemoveFromCart={(productId) => {
                  setLineItems(prev => prev.filter(item => item.productId !== productId));
                }}
                onProductAdded={() => {
                  // Focus stays in search box for rapid multi-add â€” no focus steal
                }}
              />
                  </Box>
                  <Button
